...
// }
void printSSCs(int n, struct DFS_vertex *DFS_result) {
  int *componentIndex = (int *)malloc(sizeof(int) * n);
  int numComponents = 0;

  // Initialize componentIndex array
  for (int i = 0; i < n; i++) {
    componentIndex[i] = -1;
  }

  // Assign component indices to vertices
  for (int i = 0; i < n; i++) {
    if (DFS_result[i].c == BLACK) {
      componentIndex[DFS_result[i].index] = numComponents;
    }
  }

  // Print the vertices in each SCC
  for (int i = 0; i < numComponents; i++) {
    printf("SSC %d: ", i + 1);
    for (int j = 0; j < n; j++) {
      if (componentIndex[j] == i) {
        printf("%d ", j);
      }
    }
    printf("\n");
...
int DFS_VISIT(int n, struct list *Adj_array, struct DFS_vertex *DFS_vertices,
  ...
  struct list *adjList = &Adj_array[u];
  while (adjList->next != NULL) {
    adjList = adjList->next;
    int v = adjList->vertex_index;
    if (DFS_vertices[v].c == WHITE) {
      DFS_vertices[v].pi = &DFS_vertices[u];
      time = DFS_VISIT(n, Adj_array, DFS_vertices, v, time);
    }
  }
