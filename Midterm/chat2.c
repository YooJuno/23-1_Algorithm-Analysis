...
//         strcat(so_far, "0");
//         GetHuffmanCode(codeTree->left, so_far);
//         so_far[strlen(so_far) - 1] = '\0';

//         strcat(so_far, "1");
//         GetHuffmanCode(codeTree->right, so_far);
//         so_far[strlen(so_far) - 1] = '\0';
//     } 
//     else {
//         printf("test\n");
//         strcat(code[Char2Index(codeTree->letter)], so_far);
//     }
        
// }
void GetHuffmanCode(struct zNode *codeTree, char *so_far) {
    if (codeTree->left->letter != 0x00) {
        strcpy(code[Char2Index(codeTree->letter)], so_far);
        return;
    } 
    else {
        // char temp[100] = "";
...
void Insert(struct Queue *Q, struct zNode *z) {
    ...
    // If the new node's freq is less than the front node's freq
    if (z->freq < Q->front->key->freq) {
        temp->next = Q->front;
        Q->front = temp;
        return;
    }

    // Find the node before which the new node is to be inserted and insert the
    // new node
    ...
    // If the new node's freq is greater than rear node's freq
    if (curr == NULL) {
        Q->rear = temp;
    }
}
